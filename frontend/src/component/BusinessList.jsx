  ...
  return (
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6   mt-2">
    {
       ...
       businessList?.length > 0 ? businessList?.map((item,index)=>(
        <Link to={"/details/"+item?._id} key={index} className="flex flex-col shadow-md items-baseline rounded-lg hover:shadow-lg hover:shadow-primary transition-all ease-in-out cursor-pointer hover:scale-10">
