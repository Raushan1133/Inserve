...
const [location, setLocation] = useState({ latitude: null, longitude: null });
  ...
  const fetchProviderDetails = async()=>{
    try {
      const response = await fetch(summaryApi.fetchProviderDetails.url,{
        credentials:'include'
      });
      const responseData = await response.json();
      setProviderData(responseData.data)
      setLocation({
      ...
      console.log(location)
        ...
        toast.error("Error while loading informations !");
    }
  }
  const[isLoading,setIsLoading] = useState(false);
    ...
    try {
      setIsLoading(true);
      const response =await fetch(summaryApi.updateBusinessDetails.url,{
        method:summaryApi.updateBusinessDetails.method,
        headers:{
...
//     const accessToken = import.meta.env.VITE_MAPBOX_TOKEN; // Store token in .env
//     const url = `https://api.mapbox.com/geocoding/v5/mapbox.places/${location.latitude},${location.longitude}.json?access_token=${accessToken}`;
...
// };
 ...
 // Get all categories
  ...
  const getAddressFromCoordinate = async()=>{
    const data  = await getAddressFromCoordinates(providerData?.location?.coordinates[0],providerData?.location?.coordinates[1]);
  ...
  return (
    ...
    <div className='md:mx-10 mx-5 relative'>
      <div className="flex justify-center ">
        <form className="max-w-xl shadow-2xl dark:bg-gray-900 rounded   md:p-10 p-5  flex flex-col gap-2 ">
        ...
        {/* Business details - business name */}
            ...
            <select name="category"  id="category" onChange={(e)=>handleOnChange(e)}  className='w-full cursor-pointer p-2 border border-primary rounded-sm outline-none text-black'> 
              <option value="" className=' p-2 text-center cursor-pointer' >{providerData?.category?.name}</option>
              {
            ...
            <select name="businessStartTime" id="businessStartTime" onChange={(e)=>handleOnChange(e)} value={providerData?.businessStartTime}  className='w-full cursor-pointer p-2 border border-primary rounded-sm outline-none text-black'> 
            ...
            <select value={providerData?.businessClosingTime} name="businessEndTime" id="businessEndTime" onChange={(e)=>handleOnChange(e)}  className='w-full cursor-pointer p-2 border border-primary rounded-sm outline-none text-black'> 
            ...
            <div >
                ...
                <input value={providerData?.serviceGap} type="number"
                 placeholder='ex:- 30'
                 name='serviceGap'
                 id='serviceGap'
                  className='w-full p-2 border border-primary rounded-sm outline-none text-black'
            ...
            <input type="file" name="image" id="image" className='hidden' onChange={(e)=>handlyGalleryUpload(e)} />
              ...
              <div className='grid  grid-cols-2 justify-items-center sm:grid-cols-5  gap-2'>
                {
                  providerData?.gallery?.length >= 0 ? providerData?.gallery?.map((photo,index)=>(
                    <div className='relative' key={index} ><img src={photo} alt="gallery" className='h-28 w-28 aspect-square' />
            ...
            <div>
            <Button className="w-full mt-2" disabled={isLoading} onClick={(e)=>updateBusinessDetails(e)} >{isLoading ? <Loader2Icon className='animate-spin' /> : "Update"} </Button>
            </div> 
        </form>
      </div>
